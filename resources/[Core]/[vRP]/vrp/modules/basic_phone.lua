...
function vRP.phoneCall(user_id, phone)
    ...
    if identity and dest_id then
        ...
        if dest_src then
            ...
            if vRP.request(dest_src, lang.phone.call.ask({ from }), 15) then
                -- accepted
    ...
    if user_id then
        ...
        local ch_entry = function(player, choice)
            ...
            local ch_sendpos = function(player, choice)
                -- send current position to directory entry
        ...
        for k, v in pairs(phone_directory) do
            -- add directory entries (name -> number)
            menu[k] = { ch_entry, v }
        end
