...
items["medkit"] = { "Medical Kit", "Used to reanimate unconscious people.", nil, 0.5, "prop_ld_health_pack" }
items["dirty_money"] = { "Dirty money", "Illegally earned money.", nil, 0, "bkr_prop_money_unsorted_01" }
items["repairkit"] = { "Repair Kit", "Used to repair vehicles.", nil, 0.5, "prop_tool_blowtorch" }
...
items["money"] = { "Money", "Packed money.", function(args)
    ...
    return choices
end, 0, "bkr_prop_money_wrapped_01" }
...
items["money_binder"] = { "Money binder", "Used to bind 1000$ of money.", function(args)
    ...
    choices["Bind money"] = { function(player, choice, mod)
        -- bind the money
        local user_id = vRP.getUserId(player)
        if user_id then
...
local wbody_model = function(args)
    local weapon = weapons.weapon_types[args[2]]
    if weapon then
        return weapon.model_hash
    end
end

items["wbody"] = { wbody_name, wbody_desc, wbody_choices, wbody_weight, wbody_model }
...
local wammo_choices = function(args)
    ...
    choices["Load"] = { function(player, choice)
        ...
        if user_id then
            ...
            if uweapons[args[2]] then
                -- check if the weapon is equiped
...
items["wammo"] = { wammo_name, wammo_desc, wammo_choices, wammo_weight, "prop_ld_ammo_pack_03" }
