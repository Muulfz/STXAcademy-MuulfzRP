        ...
        if ngroup then
            if ngroup._config and ngroup._config.gtype ~= nil then
                ...
                for k, v in pairs(_user_groups) do
                    -- remove all groups with the same gtype
...
function vRP.hasPermission(user_id, perm)
    ...
    if fchar == "@" then
        ...
        if #parts == 3 then
            -- decompose group.aptitude.operator
            local group = parts[1]
            ...
            if fop == "<" then
                -- less (group.aptitude.<x)
                local lvl = parseInt(string.sub(op, 2, string.len(op)))
                if alvl < lvl then
                    return true
                end
            elseif fop == ">" then
                -- greater (group.aptitude.>x)
                local lvl = parseInt(string.sub(op, 2, string.len(op)))
                if alvl > lvl then
                    return true
                end
            else
                -- equal (group.aptitude.x)
    ...
    elseif fchar == "!" then
        ...
        -- regular plain permission
        -- precheck negative permission
        local nperm = "-" .. perm
        for k, v in pairs(user_groups) do
            if v then
                -- prevent issues with deleted entry
...
-- check if the user has a specific list of permissions (just a one permission)
function vRP.hasListPermissions(user_id, perms)
    for k, v in pairs(perms) do
