...
function processSpawnController(source,statusSent,user_id)
	...
	elseif statusSent == 1 or statusSent == 0 then
		userlogin[user_id] = true
		TriggerClientEvent("ven_character:characterCreate",source)
	end
end

RegisterServerEvent("ven_character:finishedCharacter")
AddEventHandler("ven_character:finishedCharacter",function(characterNome,characterSobrenome,characterAge,currentCharacterMode)
	local source = source
...
function doSpawnPlayer(source,user_id,firstspawn)
	TriggerClientEvent("ven_character:normalSpawn",source,firstspawn)
	TriggerEvent("ven-barbershop:init",user_id)
end